...
const StudentDashboard = () => {
  ...
  const [wordBankStats, setWordBankStats] = useState({
    synonyms: { learned: 0, total: 0 },
    antonyms: { learned: 0, total: 0 },
    idioms: { learned: 0, total: 0 }
  });
  ...
  const fetchWordBankProgress = async () => {
    ...
    try {
      // Get total counts from separate tables
      ...
      setWordBankStats({
        synonyms: { learned: synonymsLearned, total: synonymsCount.count || 0 },
        antonyms: { learned: antonymsLearned, total: antonymsCount.count || 0 },
        idioms: { learned: idiomsLearned, total: idiomsCount.count || 0 }
      });
    } catch (error) {
      console.error('Error fetching word bank progress:', error);
    }
  };
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{dashboardStats.totalWordsLearned}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{dashboardStats.currentStreak} days</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 space-y-1">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">{wordBankStats.synonyms.learned}/{wordBankStats.synonyms.total}</span>
                </div>
                <Progress value={wordBankStats.synonyms.total > 0 ? (wordBankStats.synonyms.learned / wordBankStats.synonyms.total) * 100 : 0} className="h-2" />
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground">Antonyms: {wordBankStats.antonyms.learned}/{wordBankStats.antonyms.total}</span>
                  <span className="text-muted-foreground">Idioms: {wordBankStats.idioms.learned}/{wordBankStats.idioms.total}</span>
                </div>
              </div>
            </CardContent>
          </Card>

...
const Games = () => {
  ...
  const fetchGames = async () => {
    try {
      // Use quiz table to create interactive games
      const { data, error } = await supabase
        .from('quizzes')
        .select('*')
        .eq('quiz_type', 'interactive')
        .order('difficulty_level');

      if (error) throw error;
      
      // Transform quiz data to game format
      const transformedGames = (data || []).map(quiz => ({
        id: quiz.id,
        title: quiz.title,
        description: quiz.description || 'Interactive learning game',
        game_type: 'match_following' as const,
        game_data: quiz.questions,
        difficulty_level: quiz.difficulty_level || 1,
        time_limit: quiz.time_limit || 300,
        max_score: Array.isArray(quiz.questions) ? quiz.questions.length * 10 : 100
      }));
  ...
  const submitGame = async () => {
    ...
    try {
      // Use quiz_attempts table for game attempts
